...
- **就地确认、不重发**:左键放行后 claude **在同一次调用里原地继续**执行;右键拒绝(或不理睬等超时)则 claude 看到「主人没同意」后**优雅收尾**,不报错、不崩。
- **不必干等**:不想做就**右键气泡立刻拒绝**,无需等 280s 超时。
- **等确认不算超时**:确认气泡弹着时,后台调用的硬超时被**暂停**(计数器,多条待确认全部结束才恢复),你慢慢考虑也不会被误杀;超时未点的会被桌宠**自动回收**(冒一句「等太久啦先不做」并恢复计时)。
- **危险气泡独立插队首**:不和普通提醒/主动搭话混在 sticky 队列里,文案明确「左键=放行此操作」,避免你翻阅提醒时误点放行。
- **失败一律安全默认**:hook 异常/配置加载失败/超时/读不到决策 → **一律拒绝**(宁可拦错,不可放过)。唯一放行靠你显式左键点击。
